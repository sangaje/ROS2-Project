...
```bash
ros2 launch fleet_bringup follower.launch.py main_domain_id:=24
```

멤버(선택, 3번째 이상의 로봇):

```bash
ros2 launch fleet_bringup member.launch.py main_domain_id:=24
```
...
운용에서는 `system_bringup system.launch.py role:=leader
...
되돌려 보내지 않는다. SLAM 맵은 `system_bringup`의 명시적인
