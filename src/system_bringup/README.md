...
로봇마다 `field_robot.launch.py` 하나만 실행하면 된다.

- `field_robot.launch.py`: Leader, active scout, follower-scout 통합 진입점
- `system.launch.py`: 내부 역할별 전체 스택 구현
...
아래가 현재 실제 3-로봇 구성의 표준 실행 명령이다. `scout22`와
`follower21`은 둘 다 정찰 가능한 Burger다. 차이는 초기 임무뿐이다:
`scout22`는 바로 ACTIVE_SCOUT로 SLAM/Risk/RL 정찰을 수행하고,
`follower21`은 평소 FOLLOWER로 리더를 따라가다가 scout 사망이 확정되면
