...
def _apply_real_robot_policy(cli_args):
    ...
    cli_args.auto_start_nav2 = False
    external_slam = str(
        os.environ.get("TB3_RL_INFERENCE_EXTERNAL_SLAM", "0")
    ).strip().lower() in {"1", "true", "yes", "on"}
    if external_slam:
        # system_bringup owns localization/SLAM lifecycle in production.  Keep
        # the training-time /map observation path, but do not spawn or reset a
        # second SLAM backend from the inference process.
        cli_args.auto_start_slam = False
        cli_args.wait_slam_map = True
        cli_args.reset_slam_on_reset = False
        cli_args.restart_slam_on_reset = False
...
def parse_args():
    ...
    parsed_args.eval_deterministic = INFERENCE_DETERMINISTIC
    return parsed_args
...
def _space_contract(space) -> object:
    spaces_by_key = getattr(space, "spaces", None)
    if isinstance(spaces_by_key, dict):
        return {
            str(key): {
                "shape": tuple(getattr(value, "shape", ())),
                "dtype": str(getattr(value, "dtype", "")),
            }
            for key, value in sorted(spaces_by_key.items())
        }
    return {
...
def _spaces_match(left, right) -> bool:
    return _space_contract(left) == _space_contract(right)
...
def main(args=None):
    ...
    if not _spaces_match(model_obs_space, env_obs_space) or not _spaces_match(
        model_action_space, env_action_space
    ):
        ros.get_logger().error(
            "RL_POLICY_CONTRACT_MISMATCH | "
            f"model_obs={_space_contract(model_obs_space)} "
            f"env_obs={_space_contract(env_obs_space)} "
            f"model_action={_space_contract(model_action_space)} "
            f"env_action={_space_contract(env_action_space)}"
        )
        ros.stop_robot()
        ros.destroy_node()
        rclpy.shutdown()
    ...
    ros.get_logger().warn(
        "[SCOUT_RL] CONTRACT_VALID "
        f"obs={_space_contract(model_obs_space)} "
        f"action={_space_contract(model_action_space)}"
    )
    ros.get_logger().warn(
        f"[SCOUT_RL] DETERMINISTIC={INFERENCE_DETERMINISTIC} "
        f"SDE={INFERENCE_USE_SDE} ROLE_GATED=true"
    )
    ...
    ros.get_logger().info(
        "EVAL_POLICY_MODE | "
        f"deterministic={INFERENCE_DETERMINISTIC} "
        f"sde={INFERENCE_USE_SDE} "
