...
class MapVectorFeatureExtractor(BaseFeaturesExtractor):
    ...
    최신 구조:
      ...
      - vector branch:
          vector의 앞 360개 LiDAR bin은 1D CNN으로 각도 방향 local pattern을 인코딩.
    ...
    의도:
      - LiDAR 360도 range는 순서가 있는 1D circular signal이므로 MLP보다 Conv1d가 적합하다.
      - temporal CNN은 raw feature 370개를 channel로 보는 대신,
        per-step compact token의 시간 흐름을 본다.
      - map history를 replay buffer에 넣지 않고 현재 robot-centric map만 사용하므로
        메모리 증가를 피한다.
    """
    ...
    def __init__(
        ...
        # ------------------------------------------------------------------
        # 1) Map branch: weak 2D CNN + Global Average Pooling
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 2) LiDAR branch: 360-bin circular 1D signal encoder
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 3) Scalar stats branch: non-LiDAR 10-dim feature encoder
        # ------------------------------------------------------------------
        if self.stats_dim > 0:
    ...
    def _encode_lidar_flat(self, lidar_flat: torch.Tensor) -> torch.Tensor:
        # lidar_flat: (B, 360)
        lidar_flat = lidar_flat.float().unsqueeze(1)  # (B, 1, 360)
    ...
    def _encode_stats_flat(
        ...
        if self.stats_net is None or stats_flat is None:
            return None
        return self.stats_net(stats_flat.float())
    ...
    def forward(self, observations: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        vector_obs = observations["vector"].float()
