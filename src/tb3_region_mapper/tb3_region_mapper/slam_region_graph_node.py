...
class SlamRegionGraphNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('fallback_to_all_free_without_tf', True)
    ...
    def _update_region_graph(self, robot: Optional[Pose2D]):
        ...
        if self.use_reachable_only and robot_cell is not None and robot_cell in free_cells:
            reachable = self._flood_fill(robot_cell, free_cells, use8=True)
            source = 'reachable'
        else:
            reachable = set(free_cells)
            source = 'all_free_fallback'
    ...
    def _track_regions(self, raw_regions: List[RawRegion]) -> List[TrackedRegion]:
        ...
        for raw in raw_regions:
            ...
            for i, old in enumerate(prev):
                ...
                if dist > self.region_match_max_distance:
                    continue
                area_ratio = min(raw.area_m2, old.area_m2) / max(1e-9, max(raw.area_m2, old.area_m2))
                if area_ratio < self.region_match_area_ratio_min:
                    continue
                score = 2.0 * area_ratio - 0.75 * dist
                if score > best_score:
                    best_score = score
                    best_i = i
            if best_i >= 0:
    ...
    def _publish_markers(
        ...
        for r in regions:
            ...
            cube.scale.x = self.grid.resolution * 1.18
            cube.scale.y = self.grid.resolution * 1.18
            cube.scale.z = 0.025
