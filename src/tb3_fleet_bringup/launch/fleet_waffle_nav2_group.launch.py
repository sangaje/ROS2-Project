...
def _patch_model_topics(src_sdf: str, robot_name: str, model_label: str) -> str:
    ...
    out_dir = Path(tempfile.gettempdir()) / 'tb3_fleet_patched_sdf_domain_split'
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f'{robot_name}_{model_label}_patched.sdf'
...
def generate_launch_description():
    ...
    gz_sim = ExecuteProcess(cmd=[gz_sim_clean, '-r', '-v', gz_verbosity, world], output='screen', name='gz_sim_burger_waffle')
    ...
    converter = ExecuteProcess(cmd=['python3', single_twist_script, '--ros-args', '-r', '__node:=single_twist_stamped_to_twist_bridge', '-p', 'use_sim_time:=true', '-p', 'robot_name:=waffle', '-p', 'cmd_vel_topic:=/cmd_vel_stamped', '-p', 'internal_cmd_vel_topics:=/cmd_vel,/gz_cmd_vel_unstamped,/gz_cmd_vel_model_unstamped', '-p', 'cmd_republish_rate_hz:=0.0', '-p', 'watchdog_timeout_sec:=0.5', '-p', 'log_every_n_republish:=100'], output='screen', name='waffle_twist_stamped_to_twist')
    ...
    occupancy_grid = Node(
        package='cartographer_ros',
        executable='cartographer_' + 'occupancy' + '_grid' + '_node',
        name='cartographer_' + 'occupancy' + '_grid' + '_node',
        output='screen',
        parameters=[{'use_sim_time': True}],
    ...
    map_cleaner = ExecuteProcess(cmd=[
        'python3', map_cleaner_script,
        '--ros-args',
        ...
        '-p', 'fallback_pose_topics:=/leader_pose_odom_fallback',
        '-p', 'clear_radius_m:=0.26',
        '-p', 'stale_pose_sec:=2.0',
    ], output='screen', name='robot_footprint_map_free_space_filter', condition=is_slam)

    leader_pose = ExecuteProcess(cmd=['python3', tf_pose_script, '--ros-args', '-r', '__node:=leader_pose_tf_publisher', '-p', 'use_sim_time:=true', '-p', 'target_frame:=map', '-p', 'source_frame:=base_footprint', '-p', 'output_topic:=/leader_pose', '-p', 'publish_rate_hz:=10.0', '-p', 'log_every_n:=100'], output='screen', name='waffle_leader_pose_tf_publisher')

    leader_pose_fallback = ExecuteProcess(cmd=['python3', odom_pose_fallback_script, '--ros-args', '-r', '__node:=leader_pose_odom_fallback_publisher', '-p', 'use_sim_time:=true', '-p', 'odom_topic:=/odom_nav', '-p', 'output_topic:=/leader_pose_odom_fallback', '-p', 'frame_id:=map', '-p', 'initial_x:=0.0', '-p', 'initial_y:=0.0', '-p', 'initial_yaw:=0.0', '-p', 'log_every_n:=100'], output='screen', name='waffle_leader_pose_odom_fallback')
    ...
    controller_server = Node(package='nav2_controller', executable='controller_server', name='controller_server', output='screen', parameters=[nav2_slam_params], remappings=[('cmd_vel', '/cmd_vel_stamped')])
    ...
    goal_proxy_named = ExecuteProcess(cmd=['python3', goal_proxy_script, '--ros-args', '-r', '__node:=waffle_pose_to_nav2', '-p', 'use_sim_time:=true', '-p', 'goal_topic:=/waffle_goal_pose', '-p', 'action_name:=/navigate_to_pose', '-p', 'default_frame_id:=map', '-p', 'position_epsilon_m:=0.18', '-p', 'yaw_epsilon_rad:=0.35', '-p', 'retry_cooldown_sec:=2.0', '-p', 'wait_for_server_timeout_sec:=30.0', '-p', 'cancel_active_on_new_goal:=true'], output='screen', name='waffle_pose_to_nav2')

    waffle_through_poses = ExecuteProcess(cmd=[
    ...
    fleet_goal_dispatcher = ExecuteProcess(cmd=[
        ...
        '-p', 'fleet_final_goal_topic:=/fleet_final_goal',
        '-p', 'waffle_final_goal_topic:=/waffle_final_goal',
        '-p', 'burger_final_goal_topic:=/burger_final_goal',
        '-p', 'map_topic:=/map',
        '-p', 'waffle_pose_topic:=/leader_pose',
        '-p', 'burger_pose_topic:=/burger_pose',
        '-p', 'final_slots_around_goal:=true',
        '-p', 'formation_separation_m:=1.20',
        '-p', 'min_pair_distance_m:=0.85',
        '-p', 'clearance_check_radius_m:=0.70',
        '-p', 'search_rings:=4',
        '-p', 'search_angles:=20',
        '-p', 'republish_count:=3',
        '-p', 'republish_period_sec:=0.75',
        '-p', 'continuous_replan:=true',
        '-p', 'continuous_update_epsilon_m:=0.14',
        '-p', 'continuous_force_period_sec:=2.50',
        '-p', 'use_intermediate_waypoints:=true',
        '-p', 'waypoint_step_m:=0.85',
        '-p', 'waypoint_final_snap_distance_m:=0.95',
        '-p', 'waypoint_safe_search_radius_m:=0.45',
        '-p', 'collision_aware_waypoints:=true',
        '-p', 'burger_follow_gap_m:=1.00',
        '-p', 'burger_lateral_slot_m:=0.70',
        '-p', 'burger_final_hold_distance_m:=1.10',
        '-p', 'minimum_robot_waypoint_distance_m:=0.90',
        '-p', 'path_cross_avoidance_radius_m:=0.65',
        '-p', 'duplicate_position_epsilon_m:=0.20',
        '-p', 'duplicate_yaw_epsilon_rad:=0.35',
    ...
    waffle_astar_cmd = ExecuteProcess(cmd=[
        ...
        '-p', 'robot_pose_backup_topic:=/leader_pose_odom_fallback',
        '-p', 'leader_pose_topic:=/leader_pose',
        '-p', ['manual_goal_topic:=', astar_goal_topic],
        '-p', 'dynamic_obstacle_pose_topic:=/burger_pose',
        '-p', 'manual_goal_slot_dx_m:=0.0',
        '-p', 'manual_goal_slot_dy_m:=0.0',
        ...
        '-p', 'dynamic_obstacle_hard_radius_m:=0.42',
        '-p', 'dynamic_obstacle_soft_radius_m:=0.85',
        '-p', 'dynamic_obstacle_cost_weight:=8.0',
        '-p', 'peer_collision_stop_distance:=0.45',
        '-p', 'peer_collision_slow_distance:=0.78',
        '-p', 'max_linear:=0.075',
        '-p', 'min_linear:=0.02',
        '-p', 'max_angular:=0.50',
    ...
    ], output='screen', name='waffle_astar_cmd_vel_controller')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('cartographer_config_basename', default_value='cartographer_2d_lidar_odom.lua'),
        DeclareLaunchArgument('control_mode', default_value='nav2', description='nav2: Waffle uses Nav2 NavigateToPose. astar_cmd publishes /cmd_vel_stamped for debug only.'),
        ...
        DeclareLaunchArgument('slam_map_publish_period_sec', default_value='0.5'),
        ...
        LogInfo(msg='FLEET_NAV2_GROUP_READY | Waffle Domain25: Cartographer /map_raw, map cleaner /map, Nav2 group goal dispatcher.'),
        LogInfo(msg=['LOCALIZATION_MODE | ', localization_mode]),
        LogInfo(msg=['WORLD_SELECTED | ', world]),
        LogInfo(msg=['STATIC_MAP_SELECTED | ', map_yaml]),
        LogInfo(msg=['CARTOGRAPHER_CONFIG | ', cartographer_config_dir, '/', cartographer_config_basename]),
        LogInfo(msg=['WAFFLE_CONTROL_MODE | ', control_mode, ' | nav2 or astar_cmd']),
        LogInfo(msg='MAP_PIPELINE | /scan -> /scan_nav -> Cartographer /map_raw -> map cleaner /map. LaserScan is not filtered before SLAM.'),
        LogInfo(msg='USER_SELECTED_SPAWN | default Waffle=(1.38,3.49,0.0), Burger=(0.58,3.49,0.0). Based on user-selected Gazebo pose panel X=1.38, Y=3.49; Burger offset 0.80m in X to avoid overlap.'),
        gz_sim,
        ...
        TimerAction(period=7.0, actions=[leader_pose_fallback]),
        TimerAction(period=10.0, actions=[map_odom_localization, cartographer, occupancy_grid, leader_pose]),
        ...
        TimerAction(period=2.0, actions=[fleet_goal_dispatcher], condition=is_nav2_control),
        TimerAction(period=6.0, actions=[waffle_through_poses], condition=is_nav2_control),
        TimerAction(period=9.0, actions=[waffle_astar_cmd], condition=is_astar_control),
    ])
