...
def generate_launch_description():
    ...
    if not os.path.exists(default_rviz_config):
        raise RuntimeError(f'Missing stable RViz config: {default_rviz_config}')
    ...
    rviz = ExecuteProcess(
        cmd=[
            ...
            '-d', rviz_config,
            '--ros-args',
            '-r', '__node:=rviz2_debug_fleet_debug',
            '-p', 'use_sim_time:=true',
        ],
        output='screen',
        name='rviz2_debug_fleet_debug',
    )
    ...
    return LaunchDescription([
        ...
        LogInfo(msg='DEBUG_RVIZ | Aggregated RViz on selected domain. uses existing fleet_debug.rviz; /fleet_goal_pose tool included; shows /map and fleet markers. Do not bridge raw /tf.'),
        marker_node,
        rviz,
    ])
