...
def generate_launch_description():
    ...
    dispatcher_script = os.path.join(bringup_share, 'scripts', 'fleet_goal_dispatcher_direct_v63.py')
    ...
    gz_sim = ExecuteProcess(cmd=['gz', 'sim', '-r', '-v', gz_verbosity, world], output='screen', name='gz_sim_burger_waffle_v63')
    ...
    converter = ExecuteProcess(cmd=['python3', single_twist_script, '--ros-args', '-r', '__node:=single_twist_stamped_to_twist_bridge', '-p', 'use_sim_time:=true', '-p', 'robot_name:=waffle', '-p', 'cmd_vel_topic:=/cmd_vel', '-p', 'internal_cmd_vel_topics:=/gz_cmd_vel_unstamped,/gz_cmd_vel_model_unstamped', '-p', 'cmd_republish_rate_hz:=0.0', '-p', 'watchdog_timeout_sec:=0.5', '-p', 'log_every_n_republish:=100'], output='screen', name='waffle_twist_stamped_to_twist_v63')
    ...
    leader_pose = ExecuteProcess(cmd=['python3', tf_pose_script, '--ros-args', '-r', '__node:=leader_pose_tf_publisher', '-p', 'use_sim_time:=true', '-p', 'target_frame:=map', '-p', 'source_frame:=base_footprint', '-p', 'output_topic:=/leader_pose', '-p', 'publish_rate_hz:=10.0', '-p', 'log_every_n:=100'], output='screen', name='waffle_leader_pose_tf_publisher_v63')

    leader_pose_fallback = ExecuteProcess(cmd=['python3', odom_pose_fallback_script, '--ros-args', '-r', '__node:=leader_pose_odom_fallback_publisher', '-p', 'use_sim_time:=true', '-p', 'odom_topic:=/odom_nav', '-p', 'output_topic:=/leader_pose_odom_fallback', '-p', 'frame_id:=map', '-p', 'initial_x:=0.0', '-p', 'initial_y:=0.0', '-p', 'initial_yaw:=0.0', '-p', 'log_every_n:=100'], output='screen', name='waffle_leader_pose_odom_fallback_v63')
    ...
    goal_proxy_default = ExecuteProcess(cmd=['python3', goal_proxy_script, '--ros-args', '-r', '__node:=waffle_default_goal_pose_to_nav2', '-p', 'use_sim_time:=true', '-p', 'goal_pose_topic:=/goal_pose', '-p', 'navigate_action:=/navigate_to_pose', '-p', 'default_frame_id:=map', '-p', 'cancel_previous_goal:=true'], output='screen', name='waffle_default_goal_pose_to_nav2_v63')
    goal_proxy_named = ExecuteProcess(cmd=['python3', goal_proxy_script, '--ros-args', '-r', '__node:=waffle_named_goal_pose_to_nav2', '-p', 'use_sim_time:=true', '-p', 'goal_pose_topic:=/waffle_goal_pose', '-p', 'navigate_action:=/navigate_to_pose', '-p', 'default_frame_id:=map', '-p', 'cancel_previous_goal:=true'], output='screen', name='waffle_named_goal_pose_to_nav2_v63')
    ...
    fleet_goal_dispatcher = ExecuteProcess(cmd=[
        ...
        '-p', 'formation_separation_m:=0.75',
        '-p', 'min_pair_distance_m:=0.58',
        '-p', 'clearance_check_radius_m:=0.70',
        '-p', 'search_rings:=4',
        '-p', 'search_angles:=20',
        '-p', 'republish_count:=4',
        '-p', 'republish_period_sec:=0.20',
    ], output='screen', name='fleet_goal_dispatcher_v63')
    ...
    ], output='screen', name='waffle_astar_cmd_vel_controller_v63')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('control_mode', default_value='nav2', description='nav2: Waffle uses Nav2 NavigateToPose. astar_cmd: Waffle directly publishes /cmd_vel TwistStamped from A* path tracking.'),
        ...
        DeclareLaunchArgument('waffle_x', default_value='1.20'),
        DeclareLaunchArgument('waffle_y', default_value='-1.60'),
        ...
        LogInfo(msg='V63_WAFFLE_SLAM_ASTAR | Gazebo + Waffle Nav2 + Cartographer/static-map selectable + fleet goal dispatcher owner.'),
        LogInfo(msg=['V63_LOCALIZATION_MODE | ', localization_mode]),
        LogInfo(msg=['V63_WORLD_SELECTED | ', world]),
        LogInfo(msg=['V63_STATIC_MAP_SELECTED | ', map_yaml]),
        LogInfo(msg=['V63_CARTOGRAPHER_CONFIG | ', cartographer_config_dir, '/', cartographer_config_basename]),
        LogInfo(msg=['V63_WAFFLE_CONTROL_MODE | ', control_mode, ' | nav2 or astar_cmd']),
        LogInfo(msg='V63_SAFE_HOUSE_SPAWN | default Waffle=(1.20,-1.60,0.0), Burger=(0.45,-1.60,0.0). Override burger_x/y and waffle_x/y if needed.'),
        gz_sim,
        ...
        TimerAction(period=24.0, actions=[fleet_goal_dispatcher], condition=is_nav2_control),
        TimerAction(period=30.0, actions=[goal_proxy_named], condition=is_nav2_control),
