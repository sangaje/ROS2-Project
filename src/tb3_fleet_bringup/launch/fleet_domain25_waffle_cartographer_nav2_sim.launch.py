...
def generate_launch_description():
    ...
    astar_cmd_script = os.path.join(bringup_share, 'scripts', 'astar_cmd_vel_follower_direct_v48.py')
    ...
    gz_sim = ExecuteProcess(cmd=['gz', 'sim', '-r', '-v', gz_verbosity, world], output='screen', name='gz_sim_burger_waffle_v48')
    ...
    converter = ExecuteProcess(cmd=['python3', single_twist_script, '--ros-args', '-r', '__node:=single_twist_stamped_to_twist_bridge', '-p', 'use_sim_time:=true', '-p', 'robot_name:=waffle', '-p', 'cmd_vel_topic:=/cmd_vel', '-p', 'internal_cmd_vel_topics:=/gz_cmd_vel_unstamped,/gz_cmd_vel_model_unstamped', '-p', 'cmd_republish_rate_hz:=0.0', '-p', 'watchdog_timeout_sec:=0.5', '-p', 'log_every_n_republish:=100'], output='screen', name='waffle_twist_stamped_to_twist_v48')
    ...
    leader_pose = ExecuteProcess(cmd=['python3', tf_pose_script, '--ros-args', '-r', '__node:=leader_pose_tf_publisher', '-p', 'use_sim_time:=true', '-p', 'target_frame:=map', '-p', 'source_frame:=base_footprint', '-p', 'output_topic:=/leader_pose', '-p', 'publish_rate_hz:=10.0', '-p', 'log_every_n:=100'], output='screen', name='waffle_leader_pose_tf_publisher_v48')
    ...
    goal_proxy_default = ExecuteProcess(cmd=['python3', goal_proxy_script, '--ros-args', '-r', '__node:=waffle_default_goal_pose_to_nav2', '-p', 'use_sim_time:=true', '-p', 'goal_pose_topic:=/goal_pose', '-p', 'navigate_action:=/navigate_to_pose', '-p', 'default_frame_id:=map', '-p', 'cancel_previous_goal:=true'], output='screen', name='waffle_default_goal_pose_to_nav2_v48')
    goal_proxy_named = ExecuteProcess(cmd=['python3', goal_proxy_script, '--ros-args', '-r', '__node:=waffle_named_goal_pose_to_nav2', '-p', 'use_sim_time:=true', '-p', 'goal_pose_topic:=/waffle_goal_pose', '-p', 'navigate_action:=/navigate_to_pose', '-p', 'default_frame_id:=map', '-p', 'cancel_previous_goal:=true'], output='screen', name='waffle_named_goal_pose_to_nav2_v48')
    ...
    return LaunchDescription([
        ...
        LogInfo(msg='V48_DOMAIN25_WAFFLE_OWNER | Gazebo + Waffle Nav2 + Cartographer/static-map selectable + shared /map owner.'),
        LogInfo(msg=['V48_LOCALIZATION_MODE | ', localization_mode]),
        LogInfo(msg=['V48_WORLD_SELECTED | ', world]),
        LogInfo(msg=['V48_STATIC_MAP_SELECTED | ', map_yaml]),
        LogInfo(msg=['V48_CARTOGRAPHER_CONFIG | ', cartographer_config_dir, '/', cartographer_config_basename]),
        LogInfo(msg=['V48_WAFFLE_CONTROL_MODE | ', control_mode, ' | nav2 or astar_cmd']),
        LogInfo(msg='V48_SAFE_HOUSE_SPAWN | default Waffle=(-2.25,-1.75,0.0), Burger=(-3.20,-1.75,0.0). Override burger_x/y and waffle_x/y if needed.'),
        gz_sim,
        ...
        TimerAction(period=13.0, actions=[waffle_astar_cmd], condition=is_astar_control),
    ])
