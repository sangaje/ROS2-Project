...
class SingleTwistStampedToTwistBridge(Node):
    ...
    This republishes the latest command at a fixed rate so a single --once command
    is enough for Gazebo DiffDrive to visibly move until a zero command arrives.
    """
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            'SINGLE_TWIST_STAMPED_CMDVEL_READY | '
            f'robot={self.robot_name} | in={self.cmd_vel_topic} TwistStamped | '
            f'outs={self._out_topic_names} Twist | republish={self.cmd_republish_rate_hz:.1f}Hz'
        )
        ...
        if self.watchdog_timeout_sec > 0.0:
            ...
            self.get_logger().info(
                f'SINGLE_TWIST_STAMPED_WATCHDOG_DISABLED | robot={self.robot_name} | command persists until explicit zero command'
            )
    ...
    def _on_cmd(self, msg: TwistStamped) -> None:
        ...
        self.get_logger().info(
            f'SINGLE_TWIST_STAMPED_CMDVEL_RX | robot={self.robot_name} | vx={msg.twist.linear.x:.3f} wz={msg.twist.angular.z:.3f}'
        )
