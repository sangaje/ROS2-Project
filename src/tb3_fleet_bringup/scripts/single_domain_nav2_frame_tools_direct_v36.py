...
class SingleDomainNav2FrameTools(Node):
    ...
    def __init__(self) -> None:
        ...
        # `use_sim_time` may already be declared by rclpy/launch when passed via --ros-args -p use_sim_time:=true.
        # Do not let that duplicate declaration kill the frame_tools node.
        try:
        ...
        self.get_logger().info(
            'V36_REAL_NAV2_FRAME_TOOLS_READY | '
            f'robot={self.robot_name} | use_sim_time={self.get_parameter("use_sim_time").value} | odom_in={self.odom_in} odom_out={self.odom_out} | '
    ...
    def _initial_pose_tick(self) -> None:
        ...
        self.get_logger().info(
            f'V36_SIMTIME_INITIALPOSE_PUB | robot={self.robot_name} count={self.initial_count}/{self.initial_pose_repeat_count} stamp={msg.header.stamp.sec}.{msg.header.stamp.nanosec:09d} '
