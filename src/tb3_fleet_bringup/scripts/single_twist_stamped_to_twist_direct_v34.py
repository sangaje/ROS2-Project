...
class SingleTwistStampedToTwistBridge(Node):
    ...
    v34 real-Nav2 mode: by default, publish only when a TwistStamped arrives.
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            'V34_REAL_NAV2_TWIST_STAMPED_TO_TWIST_READY | '
        ...
        if self.watchdog_timeout_sec > 0.0:
            ...
            self.get_logger().info(
                f'V34_TWIST_BRIDGE_WATCHDOG_DISABLED | robot={self.robot_name} | no synthetic republish when cmd_republish_rate_hz=0'
            )
    ...
    def _on_cmd(self, msg: TwistStamped) -> None:
        ...
        self.get_logger().info(
            f'V34_TWIST_BRIDGE_CMD_RX | robot={self.robot_name} | vx={msg.twist.linear.x:.3f} wz={msg.twist.angular.z:.3f}'
        )
