...
class SingleTwistStampedToTwistBridge(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('internal_cmd_vel_topics', '/cmd_vel,/gz_cmd_vel_unstamped,/gz_cmd_vel_model_unstamped')
        ...
        if self.watchdog_timeout_sec > 0.0:
            ...
            self.get_logger().info(
                f'TWIST_BRIDGE_WATCHDOG_DISABLED | robot={self.robot_name} | no synthetic republish when cmd_republish_rate_hz=0'
            )
    ...
    def _on_cmd(self, msg: TwistStamped) -> None:
        ...
        self.get_logger().info(
            f'TWIST_BRIDGE_CMD_RX | robot={self.robot_name} | vx={msg.twist.linear.x:.3f} wz={msg.twist.angular.z:.3f}'
        )
