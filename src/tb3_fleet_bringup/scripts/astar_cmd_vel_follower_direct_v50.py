...
class AStarCmdVelFollower(Node):
    """V50 robust A* + direct TwistStamped controller.
    ...
    Important V50 change vs V48/V49:
    ...
    def __init__(self):
        ...
        self.inflation_radius_m = float(safe_declare(self, 'inflation_radius_m', 0.14))
        self.max_astar_cells = int(safe_declare(self, 'max_astar_cells', 90000))
        ...
        self.get_logger().info(
            'V50_ASTAR_CMD_VEL_CONTROLLER_READY | TwistStamped direct controller | '
            ...
            f'direct_fallback={self.direct_fallback_if_no_path}'
        )
    ...
    def build_occupied(self):
        ...
        key = (
            ...
            len(m.data), self.occupied_threshold, self.treat_unknown_as_obstacle, self.inflation_radius_m
        )
        ...
        if inflate_cells > 0 and obstacles:
            base = bytearray(occ)
            offsets = []
    ...
    def control_loop(self):
        ...
        if (self.now() - self.last_log_time).nanoseconds * 1e-9 >= self.log_period_sec:
            ...
            self.get_logger().info(
                f'V50_ASTAR_CMD | robot={self.robot_name} mode={self.target_mode}/{self.leader_goal_mode} '
