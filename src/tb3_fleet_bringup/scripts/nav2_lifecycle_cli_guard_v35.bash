...
wait_for_amcl_pose() {
  timeout 1.2 ros2 topic echo --once /amcl_pose >/tmp/v36_amcl_pose_${ROS_DOMAIN_ID}.log 2>/dev/null
  return $?
}

echo "V36_REAL_NAV2_LIFECYCLE_GUARD_READY | delay=${START_DELAY_SEC}s rounds=${ROUNDS} period=${PERIOD_SEC}s domain=${ROS_DOMAIN_ID}"
...
for i in $(seq 1 "${ROUNDS}"); do
  ...
  if wait_for_amcl_pose; then
    echo "V36_AMCL_POSE_READY | domain=${ROS_DOMAIN_ID}"
    for n in "${NAV_NODES[@]}"; do
      activate_node "$n"
    done
  else
    echo "V36_WAIT_AMCL_POSE | localization not publishing /amcl_pose yet; not forcing planner activation this round"
  fi
  ...
  for n in "${OPTIONAL_NODES[@]}"; do
    ...
    if echo "$st" | grep -q "unconfigured\|inactive"; then
      echo "V36_NAV2_OPTIONAL_SKIP_NONBLOCKING | node=${n} state=${st}"
    fi
  done
  ...
  if ros2 action info /navigate_to_pose 2>/dev/null | grep -q "/bt_navigator"; then
    ...
    echo "V36_NAV2_ACTION_PRESENT_BUT_SERVER_CHECK_FAILED | inspect: ros2 action info /navigate_to_pose"
  else
