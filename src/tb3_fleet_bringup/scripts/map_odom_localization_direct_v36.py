...
def _quat_to_yaw(q: Quaternion) -> float:
    # yaw from quaternion, robust enough for planar TB3
...
class MapOdomLocalization(Node):
    """Deterministic sim localization for no-namespace Nav2.

    Why this exists:
      In the Gazebo domain-bridge setup, AMCL sometimes becomes active but never
      publishes /amcl_pose. That leaves map->odom absent, so controller/planner/
      BT activation stalls. In this simulation we already know the spawn pose and
      Gazebo odom is stable. This node therefore owns map->odom and publishes a
      lightweight /amcl_pose-compatible pose. It does NOT publish cmd_vel and it
      does NOT replace Nav2's /navigate_to_pose action.

    TF convention:
      odom->base_footprint is published by single_domain_nav2_frame_tools.
      This node publishes map->odom = initial spawn pose.
      Therefore map->base = initial spawn pose (+) Gazebo odom.
    """
    ...
    def __init__(self) -> None:
        ...
        self.log_every_n = max(0, int(_safe_declare(self, 'log_every_n', 150)))
        ...
        self.get_logger().info(
            'V36_MAP_ODOM_LOCALIZATION_READY | '
            f'robot={self.robot_name} | owns_tf={self.map_frame}->{self.odom_frame} | '
            f'odom={self.odom_topic} | amcl_pose={self.amcl_pose_topic} | '
            f'initial=({self.initial_x:.2f},{self.initial_y:.2f},{self.initial_yaw:.2f}) | '
            'real_nav2_no_cmdvel_fallback=True'
        )
    ...
    def _map_pose_from_odom(self) -> Tuple[float, float, float]:
        ...
        myaw = self.initial_yaw + yaw_odom
        # normalize yaw
    ...
    def _tick(self) -> None:
        ...
        if self.log_every_n and self.tick_count % self.log_every_n == 0:
            x, y, yaw = self._map_pose_from_odom()
            self.get_logger().info(
                f'V36_MAP_ODOM_LOCALIZATION_TICK | robot={self.robot_name} n={self.tick_count} '
                f'map_pose=({x:.2f},{y:.2f},{yaw:.2f}) odom_seen={self.last_odom is not None}'
            )
