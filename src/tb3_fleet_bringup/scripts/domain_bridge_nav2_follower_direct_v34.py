...
class DomainBridgeNav2Follower(Node):
    ...
    No namespaces are used. Domain separation provides isolation.
    """
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            'DOMAIN_BRIDGE_NAV2_FOLLOWER_READY | '
    ...
    def _tick(self) -> None:
        if self.leader_pose is None:
            self.get_logger().warn('DOMAIN_BRIDGE_NAV2_FOLLOW_WAIT | no /leader_pose yet')
            return

        if not self.client.wait_for_server(timeout_sec=self.wait_timeout):
            self.get_logger().warn(f'DOMAIN_BRIDGE_NAV2_FOLLOW_WAIT | action server not ready: {self.navigate_action}')
            return
        ...
        self.goal_count += 1
        self.get_logger().info(f'DOMAIN_BRIDGE_NAV2_FOLLOW_GOAL_SENT | n={self.goal_count} target=({gx:.2f},{gy:.2f})')
    ...
    def _on_goal_response(self, future) -> None:
        try:
            ...
            self.get_logger().error(f'DOMAIN_BRIDGE_NAV2_FOLLOW_GOAL_ERROR | {e}')
            return
        if not handle.accepted:
            self.get_logger().warn('DOMAIN_BRIDGE_NAV2_FOLLOW_GOAL_REJECTED')
            return
        self.get_logger().info('DOMAIN_BRIDGE_NAV2_FOLLOW_GOAL_ACCEPTED')
