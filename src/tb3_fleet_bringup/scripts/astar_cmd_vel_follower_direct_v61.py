...
class AStarCmdVelFollower(Node):
    """V61 squad A* + shared-goal formation + dynamic peer avoidance TwistStamped controller.
    ...
    on /cmd_vel. It supports two stable modes:
      ...
      plan_follow: use the leader's Nav2 /plan direction and hold a slot behind it.
    ...
    def __init__(self):
        ...
        self.robot_pose_backup_topic = str(safe_declare(self, 'robot_pose_backup_topic', ''))
        ...
        self.nav2_plan_topic = str(safe_declare(self, 'nav2_plan_topic', '/plan'))
        # Optional peer pose. Used for StarCraft-style squad movement: both robots
        # receive the same clicked goal, but each A* planner treats the other robot
        # as a dynamic obstacle and the controller slows/stops before collision.
        self.dynamic_obstacle_pose_topic = str(safe_declare(self, 'dynamic_obstacle_pose_topic', ''))
        ...
        self.target_mode = str(safe_declare(self, 'target_mode', 'leader'))  # leader | manual | plan_follow
        ...
        self.plan_follow_lateral_offset_m = float(safe_declare(self, 'plan_follow_lateral_offset_m', 0.0))
        self.plan_follow_min_tangent_m = float(safe_declare(self, 'plan_follow_min_tangent_m', 0.20))
        # Manual goal slot offset. RViz still publishes one shared /goal_pose, but
        # followers can take a small formation slot near that goal so two robots do
        # not terminate at the exact same coordinate. dx/dy are in goal-frame when
        # manual_goal_slot_use_goal_yaw=true. dy<0 is the right-hand side of the
        # arrow drawn by RViz 2D Goal Pose.
        self.manual_goal_slot_dx_m = float(safe_declare(self, 'manual_goal_slot_dx_m', 0.0))
        self.manual_goal_slot_dy_m = float(safe_declare(self, 'manual_goal_slot_dy_m', 0.0))
        self.manual_goal_slot_use_goal_yaw = bool(safe_declare(self, 'manual_goal_slot_use_goal_yaw', True))
        ...
        # Dynamic peer obstacle: another robot is not part of /map, so inject its
        # current pose into the local A* occupancy/cost grid. Hard radius blocks
        # collision cells; soft radius biases the path away from the peer.
        self.dynamic_obstacle_hard_radius_m = float(safe_declare(self, 'dynamic_obstacle_hard_radius_m', 0.45))
        self.dynamic_obstacle_soft_radius_m = float(safe_declare(self, 'dynamic_obstacle_soft_radius_m', 0.85))
        self.dynamic_obstacle_cost_weight = float(safe_declare(self, 'dynamic_obstacle_cost_weight', 9.0))
        self.dynamic_obstacle_stale_sec = float(safe_declare(self, 'dynamic_obstacle_stale_sec', 1.2))
        self.peer_collision_stop_distance = float(safe_declare(self, 'peer_collision_stop_distance', 0.45))
        self.peer_collision_slow_distance = float(safe_declare(self, 'peer_collision_slow_distance', 0.80))
        ...
        self.allow_direct_goal_without_map = bool(safe_declare(self, 'allow_direct_goal_without_map', True))
        self.publish_direct_path_on_fallback = bool(safe_declare(self, 'publish_direct_path_on_fallback', True))
        ...
        self.nav2_plan: Optional[Path] = None
        self.dynamic_obstacle_pose: Optional[PoseStamped] = None
        self.robot_stamp = None
        self.robot_backup_stamp = None
        self.leader_stamp = None
        self.manual_goal_stamp = None
        self.nav2_plan_stamp = None
        self.dynamic_obstacle_stamp = None
        ...
        if self.robot_pose_backup_topic:
            self.create_subscription(PoseStamped, self.robot_pose_backup_topic, self.on_robot_pose_backup, qos)
        ...
        self.create_subscription(Path, self.nav2_plan_topic, self.on_nav2_plan, qos)
        if self.dynamic_obstacle_pose_topic:
            self.create_subscription(PoseStamped, self.dynamic_obstacle_pose_topic, self.on_dynamic_obstacle_pose, qos)
        ...
        self.get_logger().info(
            'V61_ASTAR_CMD_VEL_CONTROLLER_READY | TwistStamped direct controller | '
            f'robot={self.robot_name} mode={self.target_mode}/{self.leader_goal_mode} '
            f'map={self.map_topic} pose={self.robot_pose_topic} backup_pose={self.robot_pose_backup_topic or "none"} leader={self.leader_pose_topic} '
            f'manual={self.manual_goal_topic} nav2_plan={self.nav2_plan_topic} cmd={self.cmd_vel_topic} path={self.path_topic} '
            f'peer={self.dynamic_obstacle_pose_topic or "none"} slot=({self.manual_goal_slot_dx_m:.2f},{self.manual_goal_slot_dy_m:.2f}) '
            f'follow={self.follow_distance:.2f} plan_lat={self.plan_follow_lateral_offset_m:.2f} lookahead={self.lookahead_distance:.2f} '
            ...
            f'allow_no_map_direct={self.allow_direct_goal_without_map}'
        )
    ...
    def on_robot_pose_backup(self, msg: PoseStamped):
        self.robot_pose_backup = msg
        self.robot_backup_stamp = self.now()

    def active_robot_pose(self) -> Optional[PoseStamped]:
        if self.robot_pose is not None and self.age_ok(self.robot_stamp, self.stale_pose_sec):
            return self.robot_pose
        if self.robot_pose_backup is not None and self.age_ok(self.robot_backup_stamp, self.stale_pose_sec):
            return self.robot_pose_backup
        return None
    ...
    def dynamic_obstacle_xy(self) -> Optional[Tuple[float, float]]:
        if self.dynamic_obstacle_pose is None:
            return None
        if not self.age_ok(self.dynamic_obstacle_stamp, self.dynamic_obstacle_stale_sec):
            return None
        return (float(self.dynamic_obstacle_pose.pose.position.x), float(self.dynamic_obstacle_pose.pose.position.y))
    ...
    def on_nav2_plan(self, msg: Path):
        self.nav2_plan = msg
        self.nav2_plan_stamp = self.now()
    ...
    def _leader_goal_xy(self) -> Optional[Tuple[float, float]]:
    ...
    def _plan_follow_goal_xy(self) -> Optional[Tuple[float, float]]:
        if self.leader_pose is None or not self.age_ok(self.leader_stamp, self.stale_goal_sec):
            return None
        if self.nav2_plan is None or not self.age_ok(self.nav2_plan_stamp, self.stale_goal_sec):
            return self._leader_goal_xy()
        poses = self.nav2_plan.poses
        if len(poses) < 2:
            return self._leader_goal_xy()
        ...
        def point(i: int) -> Tuple[float, float]:
            p = poses[max(0, min(len(poses) - 1, i))].pose.position
            return float(p.x), float(p.y)

        x0, y0 = point(best_i)
        tx, ty = 0.0, 0.0
        for j in range(best_i + 1, len(poses)):
            x1, y1 = point(j)
            tx = x1 - x0
            ty = y1 - y0
            if math.hypot(tx, ty) >= self.plan_follow_min_tangent_m:
                break
        if math.hypot(tx, ty) < self.plan_follow_min_tangent_m:
        ...
        ux = tx / norm
        uy = ty / norm
        # Behind the leader along the Waffle Nav2 plan tangent. Optional lateral
        # offset gives a small shoulder slot without changing the shared route.
        gx = lx - self.follow_distance * ux - self.plan_follow_lateral_offset_m * uy
        gy = ly - self.follow_distance * uy + self.plan_follow_lateral_offset_m * ux
        return (gx, gy)
    ...
    def target_pose_xy(self) -> Optional[Tuple[float, float]]:
        if self.target_mode == 'manual':
            if self.manual_goal is None or not self.age_ok(self.manual_goal_stamp, self.stale_goal_sec):
                return None
            gx = float(self.manual_goal.pose.position.x)
            gy = float(self.manual_goal.pose.position.y)
            if abs(self.manual_goal_slot_dx_m) > 1e-6 or abs(self.manual_goal_slot_dy_m) > 1e-6:
                yaw = yaw_from_quat(self.manual_goal.pose.orientation) if self.manual_goal_slot_use_goal_yaw else 0.0
                dx = self.manual_goal_slot_dx_m
                dy = self.manual_goal_slot_dy_m
                gx += dx * math.cos(yaw) - dy * math.sin(yaw)
                gy += dx * math.sin(yaw) + dy * math.cos(yaw)
            return (gx, gy)

        if self.target_mode in ('plan_follow', 'nav2_plan'):
            return self._plan_follow_goal_xy()
    ...
    def build_occupied(self):
        ...
        if m is None:
            return None
        dyn_xy = self.dynamic_obstacle_xy()
        dyn_grid = self.world_to_grid(dyn_xy[0], dyn_xy[1]) if dyn_xy is not None else None
        key = (
            ...
            dyn_grid, self.dynamic_obstacle_hard_radius_m, self.dynamic_obstacle_soft_radius_m,
            self.dynamic_obstacle_cost_weight,
        )
        ...
        # Dynamic peer obstacle. This is deliberately added after static-map
        # inflation so the other robot is treated like a temporary moving obstacle.
        # It is only used when a fresh peer pose exists and lies inside the current map.
        if dyn_grid is not None:
            px, py = dyn_grid
            hard_dyn = int(math.ceil(self.dynamic_obstacle_hard_radius_m / max(1e-6, m.info.resolution)))
            soft_dyn = int(math.ceil(self.dynamic_obstacle_soft_radius_m / max(1e-6, m.info.resolution)))
            if hard_dyn > 0:
                r2 = hard_dyn * hard_dyn
                for dy in range(-hard_dyn, hard_dyn + 1):
                    for dx in range(-hard_dyn, hard_dyn + 1):
                        if dx * dx + dy * dy <= r2:
                            nx, ny = px + dx, py + dy
                            if 0 <= nx < w and 0 <= ny < h:
                                occ[ny * w + nx] = 1
            if soft_dyn > 0 and self.dynamic_obstacle_cost_weight > 0.0:
                r = soft_dyn
                for dy in range(-r, r + 1):
                    for dx in range(-r, r + 1):
                        d = math.hypot(dx, dy)
                        if d <= r:
                            ...
                            if 0 <= nx < w and 0 <= ny < h:
                                ni = ny * w + nx
                                if not occ[ni]:
                                    c = self.dynamic_obstacle_cost_weight * (1.0 - d / max(1.0, float(r)))
                                    if c > cost[ni]:
                                        cost[ni] = c
    ...
    def publish_direct_fallback_path(self, robot_xy: Tuple[float, float], goal_xy: Tuple[float, float]) -> None:
        # For live SLAM exploration, the goal can be outside the current occupancy grid.
        # Publish a simple visual target path so RViz shows intent while the real A* path
        # is rebuilt as Cartographer expands /map.
        if self.publish_direct_path_on_fallback:
            self.publish_path([robot_xy, goal_xy])

    def maybe_replan(self, goal_xy: Tuple[float, float], robot_xy: Tuple[float, float]) -> str:
        if self.map_msg is None:
            self.path_world = []
            if self.allow_direct_goal_without_map:
                self.publish_direct_fallback_path(robot_xy, goal_xy)
                return 'NO_MAP_DIRECT_EXPLORE'
            return 'NO_MAP'
        ...
        if start is None or goal is None:
            self.path_world = []
            if self.direct_fallback_if_no_path or self.allow_direct_goal_without_map:
                self.publish_direct_fallback_path(robot_xy, goal_xy)
                return f'GRID_FAIL_DIRECT_EXPLORE start={start} goal={goal}'
        ...
        if start2 is None or goal2 is None:
            ...
            if self.direct_fallback_if_no_path or self.allow_direct_goal_without_map:
                self.publish_direct_fallback_path(robot_xy, goal_xy)
                return f'NO_FREE_DIRECT_EXPLORE start2={start2} goal2={goal2}'
            if self.publish_empty_path_on_fail:
                self.publish_path([])
        ...
        if not path_grid:
            self.path_world = []
            if self.direct_fallback_if_no_path:
                self.publish_direct_fallback_path(robot_xy, goal_xy)
                return 'NO_PATH_DIRECT_EXPLORE'
            if self.publish_empty_path_on_fail:
                self.publish_path([])
            return 'NO_PATH'
    ...
    def control_loop(self):
        pose_msg = self.active_robot_pose()
        if pose_msg is None:
            self.set_status('STOP_STALE_ROBOT_POSE')
            self.stop()
            return
        map_ok = self.map_msg is not None and self.age_ok(self.map_stamp, self.stale_map_sec)
        if not map_ok and not self.allow_direct_goal_without_map:
        ...
        rx = float(pose_msg.pose.position.x)
        ry = float(pose_msg.pose.position.y)
        ryaw = yaw_from_quat(pose_msg.pose.orientation)
        robot_xy = (rx, ry)
        ...
        if not self.path_world and not self.direct_fallback_if_no_path and not self.allow_direct_goal_without_map:
        ...
        peer_xy = self.dynamic_obstacle_xy()
        if peer_xy is not None:
            peer_dist = math.hypot(peer_xy[0] - rx, peer_xy[1] - ry)
            if peer_dist < self.peer_collision_stop_distance:
                linear = 0.0
                angular = 0.0
                target_src += '+PEER_STOP'
            elif peer_dist < self.peer_collision_slow_distance:
                scale = clamp((peer_dist - self.peer_collision_stop_distance) / max(1e-6, self.peer_collision_slow_distance - self.peer_collision_stop_distance), 0.15, 1.0)
                linear *= scale
                target_src += '+PEER_SLOW'
        ...
        if self.last_log_time is None or log_dt is None or log_dt >= self.log_period_sec:
            ...
            self.get_logger().info(
                f'V61_ASTAR_CMD | robot={self.robot_name} mode={self.target_mode}/{self.leader_goal_mode} '
