...
class AStarCmdVelFollower(Node):
    ...
    def __init__(self):
        ...
        # v49: For follower robots, yaw-based 'behind leader' goals are fragile in SLAM maps.
        # line_of_sight chooses a point on the segment from robot to leader, keeping the follower
        # behind the leader without depending on leader yaw.
        self.leader_goal_mode = str(safe_declare(self, 'leader_goal_mode', 'line_of_sight'))  # line_of_sight | leader_yaw
        self.follow_distance = float(safe_declare(self, 'follow_distance', 0.70))
        ...
        self.get_logger().info(
            'V49_ASTAR_CMD_VEL_CONTROLLER_READY | direct /cmd_vel TwistStamped controller | '
            ...
            f'follow_distance={self.follow_distance:.2f} leader_goal_mode={self.leader_goal_mode} lookahead={self.lookahead_distance:.2f}'
        )
    ...
    def target_pose_xy(self) -> Optional[Tuple[float, float]]:
        ...
        if self.leader_goal_mode == 'leader_yaw':
        ...
        # v49 default: line-of-sight follower target.
        # This avoids the common failure where leader yaw points across a wall or into unknown space.
        ...
        # Already close enough: use current pose so control_loop falls into tolerance/stop.
        if dist <= self.follow_distance:
            return (rx, ry)

        ux = vx / max(1e-6, dist)
        uy = vy / max(1e-6, dist)
        gx = lx - self.follow_distance * ux
        gy = ly - self.follow_distance * uy
        return (gx, gy)
    ...
    def maybe_replan(self, goal_xy: Tuple[float, float]):
        ...
        if start is None or goal is None:
            ...
            if (self.now() - self.last_log_time).nanoseconds * 1e-9 >= self.log_period_sec:
                self.get_logger().warn(
                    f'ASTAR_REPLAN_GRID_FAIL | start={start} goal={goal} robot=({rx:.2f},{ry:.2f}) goal_xy=({goal_xy[0]:.2f},{goal_xy[1]:.2f}) map_origin=({self.map_msg.info.origin.position.x:.2f},{self.map_msg.info.origin.position.y:.2f}) size=({self.map_msg.info.width},{self.map_msg.info.height}) res={self.map_msg.info.resolution:.3f}'
                )
            return
        ...
        if start2 is None or goal2 is None:
            self.path_world = []
            if (self.now() - self.last_log_time).nanoseconds * 1e-9 >= self.log_period_sec:
                self.get_logger().warn(f'ASTAR_REPLAN_NO_FREE_CELL | start={start} start2={start2} goal={goal} goal2={goal2}')
            return
        path_grid = self.astar(start2, goal2, occ)
        if not path_grid:
            self.path_world = []
            if (self.now() - self.last_log_time).nanoseconds * 1e-9 >= self.log_period_sec:
                self.get_logger().warn(f'ASTAR_REPLAN_NO_PATH | start2={start2} goal2={goal2}')
            return
    ...
    def control_loop(self):
        ...
        if (self.now() - self.last_log_time).nanoseconds * 1e-9 >= self.log_period_sec:
            ...
            self.get_logger().info(
                f'ASTAR_CMD | mode={self.target_mode}/{self.leader_goal_mode} goal=({goal_xy[0]:.2f},{goal_xy[1]:.2f}) '
