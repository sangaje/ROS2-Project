...
class RobotFootprintMapFreeSpaceFilter(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('clear_radius_m', 0.32)
        ...
        self.get_logger().info(
            f'V72_ROBOT_FOOTPRINT_MAP_FREE_SPACE_FILTER_READY | '
    ...
    def _on_map(self, msg: OccupancyGrid) -> None:
        ...
        if self.publish_count == 1:
            self.get_logger().info(
                f'V72_MAP_FOOTPRINT_CLEAN_FIRST_MAP | passthrough_ok=1 poses_used={used} cleared_cells={cleared} '
                f'size={msg.info.width}x{msg.info.height} res={res:.3f} out={self.map_out_topic}'
            )
        if self.publish_count % self.log_every_n == 0:
            self.get_logger().info(
                f'V72_MAP_FOOTPRINT_CLEAN | poses_used={used} cleared_cells={cleared} '
