# TB3 Multi Gazebo + RViz Click Goal / Auto Patrol / Rescue Test
...
```bash
ros2 launch tb3_multi multi_tb3_gz.launch.py
```

Auto patrol/rescue mode:

```bash
ros2 launch tb3_multi multi_tb3_gz.launch.py auto_patrol:=true
```
...
```bash
ros2 topic pub --once /tb3_multi/fail_robot std_msgs/msg/String "{data: 'burger2'}"
```

Recover one robot:

```bash
ros2 topic pub --once /tb3_multi/recover_robot std_msgs/msg/String "{data: 'burger1'}"
```

Recover all robots and resume patrol:

```bash
ros2 topic pub --once /tb3_multi/recover_robot std_msgs/msg/String "{data: 'all'}"
```

You can also clear failures through `/tb3_multi/fail_robot`:

```bash
ros2 topic pub --once /tb3_multi/fail_robot std_msgs/msg/String "{data: 'clear'}"
```
...
| Auto patrol/rescue mode | `/tb3_multi/mode` |
| Rescue event message | `/tb3_multi/rescue_event` |
| Manual failure trigger | `/tb3_multi/fail_robot` |
| Manual recovery trigger | `/tb3_multi/recover_robot` |
| Map | `/map` |
...
1. Manual simulation through `/tb3_multi/fail_robot`.
...
4. RViz displays failure/rescue markers on `/tb3_multi/rescue_markers`.
